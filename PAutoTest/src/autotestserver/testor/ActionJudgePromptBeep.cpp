...
bool ActionJudgePromptBeep::run(std::string casename, int seq)
{
    ...
    if (! vad.doVadToFile(part1path, _timeout,prompt->m_iDataLen)) {
        return false;
    }
    WavFile *ttsrecordfile = new WavFile();
    ttsrecordfile->openR(part1path.c_str(), g_configInfo.sampling_rate);
    ...
    beeprecordfile->openR(part2path.c_str(), g_configInfo.sampling_rate);
...
//======================Judge first_Prompt ===========================
    ...
    std::string part1_result = ProcessSelelct(part1path.c_str(), _filePath.c_str());
    Json::Reader reader1;
    Json::Value root1;
...
//=========================Judge last_beep ===========================
    ...
    std::string part2_result = ProcessBeep(part2path.c_str(), type->getBeepType());
    Json::Reader reader2;
    Json::Value root2;
    if (reader2.parse(part2_result, root2))
    {
        matchResult2 = root2["matchResult"].asBool();
    }
    LOG("ProcessSelelct t2:%d matchResult3:%d\n", type->getBeepType(), matchResult2);
