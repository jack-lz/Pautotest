...
//  unsigned int rate = 16000;
  set_param_ret =
      snd_pcm_hw_params_set_rate_near(pcm_handle, pcm_params, &g_configInfo.sampling_rate, nullptr);
  if (set_param_ret < 0) {
  ...
  alsaThread.reset(new std::thread([this, pcm_handle]() {
    std::unique_lock<std::mutex> runLock(isRunningMutex);
    while (isRunning) {
      std::unique_lock<std::mutex> dataLock(audioDataMutex);
//      while (audioData.size() == 0 && isRunning) {
      while (audioData.size() == 0) {
        audioDataCv.wait_for(dataLock, std::chrono::milliseconds(100));
      }
//      if (!isRunning) {
//        break;
//      }
...
//  isRunning = false;
  ...
  while (isRunning) {
    isRunningCv.wait_for(lock, std::chrono::milliseconds(100));
  }
  alsaThread->join();
